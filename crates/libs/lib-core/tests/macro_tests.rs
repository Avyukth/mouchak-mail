...
async fn test_builtin_macro_contents() {
    ...
    let project_id = ProjectBmc::create(c, mm, "macro-content-test", "/path/to/project").await.unwrap();
    ...
    let prepare_thread = MacroDefBmc::get_by_name(c, mm, project_id, "prepare_thread").await.unwrap();
    ...
    let file_res = MacroDefBmc::get_by_name(c, mm, project_id, "file_reservation_cycle").await.unwrap();
    ...
    let contact = MacroDefBmc::get_by_name(c, mm, project_id, "contact_handshake").await.unwrap();
    ...
    let broadcast = MacroDefBmc::get_by_name(c, mm, project_id, "broadcast_message").await.unwrap();
...
async fn test_builtin_macros_per_project_isolation() {
    ...
    // Create two projects
    let project1_id = ProjectBmc::create(c, mm, "project-1", "/path/1").await.unwrap();
    let project2_id = ProjectBmc::create(c, mm, "project-2", "/path/2").await.unwrap();
    ...
    // No ID overlap
    for id in &p1_ids {
        assert!(!p2_ids.contains(id), "Macro IDs should be unique across projects");
    }
...
async fn test_builtin_macros_have_timestamps() {
    ...
    let project_id = ProjectBmc::create(c, mm, "timestamp-test", "/path/ts").await.unwrap();
    let macros = MacroDefBmc::list(c, mm, project_id).await.unwrap();

    for macro_def in macros {
        // Verify timestamps are not default/zero
        assert!(macro_def.created_ts.and_utc().timestamp() > 0,
            "Macro {} should have valid created_ts", macro_def.name);
        assert!(macro_def.updated_ts.and_utc().timestamp() > 0,
            "Macro {} should have valid updated_ts", macro_def.name);
    }
}
...
async fn test_macro_registration_order() {
    ...
    let project_id = ProjectBmc::create(c, mm, "order-test", "/path/order").await.unwrap();
    ...
    assert_eq!(names, sorted_names, "Macros should be returned in alphabetical order");
}
...
async fn test_register_custom_macro() {
    ...
    let project_id = ProjectBmc::create(c, mm, "custom-reg", "/path/custom").await.unwrap();
    ...
    let retrieved = MacroDefBmc::get_by_name(c, mm, project_id, "deploy_workflow").await.unwrap();
...
async fn test_list_registered_macros() {
    ...
    let project_id = ProjectBmc::create(c, mm, "list-test", "/path/list").await.unwrap();
    ...
    for macro_def in &all_macros {
        ...
        assert_eq!(macro_def.project_id, project_id, "Macro should belong to correct project");
        assert!(!macro_def.name.is_empty(), "Macro name should not be empty");
        assert!(!macro_def.description.is_empty(), "Macro description should not be empty");
        assert!(!macro_def.steps.is_empty(), "Macro should have at least one step");
    }
...
async fn test_unregister_macro() {
    ...
    let project_id = ProjectBmc::create(c, mm, "unreg-test", "/path/unreg").await.unwrap();
    ...
    // Delete it
    let deleted = MacroDefBmc::delete(c, mm, project_id, "temp_macro").await.unwrap();
    ...
    let deleted_again = MacroDefBmc::delete(c, mm, project_id, "temp_macro").await.unwrap();
    assert!(!deleted_again, "Deleting non-existent macro should return false");
}
...
async fn test_macro_get_nonexistent() {
    ...
    let project_id = ProjectBmc::create(c, mm, "nonexist-test", "/path/nonexist").await.unwrap();

    // Try to get a macro that doesn't exist
    let result = MacroDefBmc::get_by_name(c, mm, project_id, "does_not_exist").await;
    assert!(result.is_err(), "Getting non-existent macro should return error");
}
...
async fn test_builtin_macros_parameter_structure() {
    ...
    let project_id = ProjectBmc::create(c, mm, "param-test", "/path/param").await.unwrap();
    ...
    for step in &prepare.steps {
        assert!(step.get("description").is_some(), "Each step should have description");
    }
}
...
async fn test_macro_steps_serialization() {
    ...
    let project_id = ProjectBmc::create(c, mm, "serial-test", "/path/serial").await.unwrap();
    ...
    let retrieved = MacroDefBmc::get_by_name(c, mm, project_id, "complex_macro").await.unwrap();
    assert_eq!(retrieved.steps.len(), 1);
    assert_eq!(retrieved.steps[0]["tool"], "complex_tool");
    assert_eq!(retrieved.steps[0]["params"]["nested"]["deeply"]["nested"], "value");
