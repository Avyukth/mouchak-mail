...
use mouchak_mail_core::{self, Ctx};
...
pub async fn register_agent(
    ...
    let agent_id = mouchak_mail_core::model::agent::AgentBmc::create(&ctx, mm, agent_c).await?;

    // Fetch the full agent to return
    let agent = mouchak_mail_core::model::agent::AgentBmc::get(&ctx, mm, agent_id).await?;
...
pub async fn send_message(
    ...
    let message_id = mouchak_mail_core::model::message::MessageBmc::create(&ctx, mm, msg_c).await?;
...
pub async fn list_all_projects(
    ...
    let projects = mouchak_mail_core::model::project::ProjectBmc::list_all(&ctx, mm).await?;
...
fn find_path_conflicts(
    ...
    active_reservations: &[mouchak_mail_core::model::file_reservation::FileReservation],
...
pub async fn list_all_locks(State(app_state): State<AppState>) -> crate::error::Result<Response> {
    ...
    for res in reservations {
        // Get project slug
        let project = mouchak_mail_core::model::project::ProjectBmc::get(&ctx, mm, res.project_id)
            .await
            .ok();
        ...
        // Get agent name
        let agent = mouchak_mail_core::model::agent::AgentBmc::get(&ctx, mm, res.agent_id)
            .await
            .ok();
...
pub async fn get_thread(
    ...
    for msg in messages {
        let recipients =
            mouchak_mail_core::model::message::MessageBmc::get_recipients(&ctx, mm, msg.id).await?;
...
pub async fn reply_message(
    ...
    let original_msg =
        mouchak_mail_core::model::message::MessageBmc::get(&ctx, mm, payload.message_id).await?;
...
pub async fn get_project_info(
    ...
    // Count messages
    let message_count =
        mouchak_mail_core::model::project::ProjectBmc::count_messages(&ctx, mm, project.id).await?;
...
pub async fn update_agent_profile(
    ...
    mouchak_mail_core::model::agent::AgentBmc::update_profile(&ctx, mm, agent.id, update).await?;
...
pub async fn request_contact(
    ...
    let from_project = mouchak_mail_core::model::project::ProjectBmc::get_by_identifier(
        &ctx,
        mm,
        &payload.from_project_slug,
    )
    .await?;
    let from_agent = mouchak_mail_core::model::agent::AgentBmc::get_by_name(
        &ctx,
        mm,
        from_project.id,
...
pub async fn respond_contact(
    ...
    mouchak_mail_core::model::agent_link::AgentLinkBmc::respond_contact(
        &ctx,
        mm,
        payload.link_id,
        payload.accept,
...
pub async fn renew_build_slot(
    ...
    let new_expires = mouchak_mail_core::model::build_slot::BuildSlotBmc::renew(
        &ctx,
        mm,
        payload.slot_id,
        payload.ttl_seconds,
...
pub async fn release_build_slot(
    ...
    mouchak_mail_core::model::build_slot::BuildSlotBmc::release(&ctx, mm, payload.slot_id).await?;
...
pub async fn macro_start_session(
    ...
    for pattern in &payload.patterns {
        let res_c = mouchak_mail_core::model::file_reservation::FileReservationForCreate {
            project_id: project.id,
            agent_id,
    ...
    if payload.action == "reserve" {
        ...
        for pattern in &payload.patterns {
            if let Some(released_id) =
                mouchak_mail_core::model::file_reservation::FileReservationBmc::release_by_path(
                    &ctx,
                    mm,
                    project.id.get(),
                    agent.id.get(),
...
async fn call_openai_summarize(
    messages: &[mouchak_mail_core::model::message::Message],
...
pub async fn commit_archive(
    ...
    let commit_id = mouchak_mail_core::model::export::ExportBmc::commit_archive(
        &ctx,
        mm,
        &payload.project_slug,
        &payload.message,
...
pub async fn list_pending_reviews(
    ...
    let rows = mouchak_mail_core::model::message::MessageBmc::list_pending_reviews(
        &ctx,
        mm,
...
pub async fn list_archive_commits(
    ...
    let filter = if params.author.is_some() || params.path.is_some() {
        Some(mouchak_mail_core::model::archive_browser::CommitFilter {
    ...
    let commits = mouchak_mail_core::model::archive_browser::ArchiveBrowserBmc::list_commits(
        &ctx,
        mm,
        filter,
        params.limit,
...
pub async fn list_archive_files(
    ...
    let files = mouchak_mail_core::model::archive_browser::ArchiveBrowserBmc::list_files_at(
        &ctx, mm, &sha, &dir_path,
    )
    .await?;
...
pub async fn get_archive_file_content(
    ...
    let content = mouchak_mail_core::model::archive_browser::ArchiveBrowserBmc::file_content_at(
        &ctx,
        mm,
        &sha,
        &params.path,
...
pub async fn get_archive_activity(
    ...
    let activity = mouchak_mail_core::model::archive_browser::ArchiveBrowserBmc::activity_timeline(
        &ctx, mm, since, until,
    )
    .await?;
