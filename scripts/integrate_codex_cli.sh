#!/usr/bin/env bash
# integrate_codex_cli.sh - Configure Codex CLI (OpenAI) to use MCP Agent Mail
...
print_header() {
    echo ""
    echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║${NC}     MCP Agent Mail - Codex CLI Integration                 ${BLUE}║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
detect_codex() {
    log_info "Detecting Codex CLI installation..."
...
update_codex_config() {
    log_info "Updating Codex config: $CODEX_CONFIG"
    ...
    if [[ -f "$CODEX_CONFIG" ]]; then
        ...
        if echo "$existing" | jq -e '.mcpServers' &> /dev/null; then
            echo "$existing" | jq --argjson config "$mcp_config" \
                ".mcpServers[\"$MCP_SERVER_NAME\"] = \$config" > "$CODEX_CONFIG"
        else
            echo "$existing" | jq --argjson config "$mcp_config" \
                ". + {mcpServers: {\"$MCP_SERVER_NAME\": \$config}}" > "$CODEX_CONFIG"
        fi
    ...
    log_success "Updated $CODEX_CONFIG"
}
...
verify_installation() {
    ...
    if [[ -f "$CODEX_CONFIG" ]]; then
        if jq -e ".mcpServers[\"$MCP_SERVER_NAME\"]" "$CODEX_CONFIG" &> /dev/null; then
            log_success "Config verified: $CODEX_CONFIG"
        else
...
print_summary() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Restart Codex CLI to load the new configuration"
    echo "  2. STDIO mode: Codex will spawn the server automatically"
    echo ""
}
...
Configure Codex CLI to use MCP Agent Mail.
